...
export async function POST(request: Request) {
  try {
    ...
    const { sessionID, roundIndex, pairIndex, winnerImdbId, voterSocketId } =
      body;

    const missingField = !sessionID
      ? "sessionID"
      : winnerImdbId === undefined
        ? "winnerImdbId"
        : pairIndex === undefined
          ? "pairIndex"
    ...
    if (missingField) {
      console.error(
        "[tournament/vote] Missing required field:",
        missingField,
        "| body:",
        body,
      );
      return Response.json(
        { success: false, message: `Missing required field: ${missingField}` },
        { status: 400 },
      );
    }
    ...
    if (!tournament) {
      return Response.json(
        { success: false, message: "No tournament in progress" },
        { status: 400 },
      );
    }
    ...
    if (tournament.roundIndex !== roundIndex) {
      return Response.json(
        { success: false, message: "Round mismatch" },
        { status: 409 },
      );
    }
    ...
    if (pairIndex < 0 || pairIndex >= tournament.pairs.length) {
      return Response.json(
        { success: false, message: "Invalid pair index" },
        { status: 400 },
      );
    }
    ...
    if (!wasSet) {
      ...
      return Response.json(
        { success: false, message: "Already voted for this pair" },
        { status: 409 },
      );
    }
    ...
    for (let i = 0; i < pairsCount; i++) {
      const votesForPair = Object.keys(allVotes).filter((k) =>
        k.startsWith(`${i}:`),
      ).length;
    ...
    for (let i = 0; i < pairsCount; i++) {
      ...
      const { winner, loser, loserVotes } = resolvePair(
        pairVotes,
        movieA,
        movieB,
      );
